...
double Particle::update(std::vector<double> collectiveLowestErrorPosition, double targetValue, std::vector<double> input) {
...
double Particle::updateMultiple(std::vector<double> collectiveLowestErrorPosition, std::vector<double> targetValues, std::vector<std::vector<double>> inputs) {
	...
	// evaluate error in new position
	double error = evaluateMultiple(targetValues, inputs);
	if (error < lowestError_) {
		lowestError_ = error;
		lowestErrorPosition_ = currentPosition_;
		updatesSinceMinimum_ = 0;
	...
	// give the particle a slight chance to be reinitialized each run; it is more likely when the particle hasn't reduced its error in a while
	if (generateRandomNumber(0, 1) < 0.001 * updatesSinceMinimum_) {
		randomizePosition();
	}
...
// calculate error
double Particle::evaluate(double targetValue, std::vector<double> input) {
	double result = 0;
...
double Particle::evaluateMultiple(std::vector<double> targetValues, std::vector<std::vector<double>> inputs) {
	
	double errors = 0;
	for (unsigned int i = 0; i < inputs.size(); ++i) {
		double result = 0;
		for (unsigned int j = 0; j < inputs[i].size(); ++j) {
			result += currentPosition_[i] * inputs[i][j];
		}


		errors += abs(result - targetValues[i]);
